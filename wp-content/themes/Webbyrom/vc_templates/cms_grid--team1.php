        ...
        if(isset($atts['show_more']) && $atts['show_more']):  
            ...
            wp_localize_script(
                ...
                array(
                    ...
                    'masonry' => $atts['layout'],
                    'loadmore_text' => esc_html__( 'Load more', 'Webbyrom' )
                )
            );
...
?>
<div class="cms-grid-wraper cms-grid-team-1 <?php echo esc_attr($atts['template']);?>" id="<?php echo esc_attr($atts['html_id']);?>"> 
    <div class="row <?php echo esc_attr($atts['grid_class']);?>">
        <?php
        $posts = $atts['posts'];
        $img_size = !empty($atts['img_size']) ? $atts['img_size']  : '370x440';
                ...
                <div class="cms-grid-item-wrap">
                        ...
                        <?php if (has_post_thumbnail()): ?>
                            <div class="item-media" style=" <?php echo esc_attr($radius);?>" >   
                                    ...
                                    <div class="media-links">
                                        <div class="cms-grid-media"><?php echo Webbyrom_get_image_croped(get_post_thumbnail_id(get_the_ID()), $img_size); ?></div>   
                                    </div>
                    ...
                    <div class="item-content text-center">
                        ...
                        <?php if ( (!empty($atts['show_email']) && $atts['show_email'] == 1) || (!empty($atts['show_phone']) && $atts['show_phone'] == 1) || (!empty($atts['show_location']) && $atts['show_location'] == 1) ): ?>
                            <ul class="team-contact" style="<?php echo esc_attr($color); ?>">
                                <?php if (!empty($atts['show_email']) && $atts['show_email'] == 1): ?>
                                     <?php if (!empty($opt_meta_options['team_email'])): ?>
                                        <li><span class="title" style="<?php echo esc_attr($color); ?>"><?php echo esc_html__('EMAIL ADDRESS: ','Webbyrom') ?></span><span><?php echo esc_attr( $opt_meta_options['team_email'] ); ?></span></li>
                                    <?php endif; ?>
                                <?php endif ?>
                                <?php if (!empty($atts['show_phone']) && $atts['show_phone'] == 1): ?>
                                    <?php if (!empty($opt_meta_options['team_phone'])): ?>
                                        <li><span class="title" style="<?php echo esc_attr($color); ?>"><?php echo esc_html__('PHONE NO: ','Webbyrom') ?></span><span><?php echo esc_attr( $opt_meta_options['team_phone'] ); ?></span></li>
                                    <?php endif; ?>
                                <?php endif; ?>
                                <?php if (!empty($atts['show_location']) && $atts['show_location'] == 1): ?>
                                    <?php if (!empty($opt_meta_options['team_location'])): ?>
                                        <li><span class="title" style="<?php echo esc_attr($color); ?>"><?php echo esc_html__('LOCATION: ','Webbyrom') ?></span><span><?php echo esc_attr( $opt_meta_options['team_location'] ); ?></span></li>
                                    <?php endif; ?>
                                <?php endif; ?>
                            </ul>
                        <?php endif; ?>

        ...
        if(isset($atts['show_more']) && $atts['show_more']):  
            ...
            wp_localize_script(
                ...
                array(
                    ...
                    'masonry' => $atts['layout'],
                    'loadmore_text' => esc_html__( 'View More Post', 'Webbyrom' )
                )
            );
    ...
    <div class="row cms-grid <?php echo esc_attr($atts['grid_class']);?>">
        ...
        while($posts->have_posts()){
            ...
            <div class="<?php echo esc_attr($class_item);?> <?php echo esc_attr($has_readmore); ?>" data-groups='[<?php echo implode(',', $groups);?>]'>
                <article <?php post_class('Webbyrom-cms-blog2'); ?>>
                    <div class="blog-outer">    
                        <div class="blog-wrap blog-column">
                            <div class="blog-content">
                                <?php if ( has_post_thumbnail()): ?>
                                    <div class="post-thumbnail">
                                        <a href="<?php echo esc_url(get_permalink()) ; ?>"><?php echo Webbyrom_get_image_croped(get_post_thumbnail_id(get_the_ID()),$image_size); ?></a>       
                                    </div>
                                <?php endif; ?> 
                                <header class="entry-header">
                                    <?php the_title( '<h5 class="entry-title"><a href="' . esc_url( get_permalink() ) . '" rel="bookmark">', '</a></h5>' ); ?><?php if (!empty($atts['show_categories']) && $atts['show_categories'] == 1): ?>
                                        <div class="entry-category">
                                            <span class="title-cate"><?php echo esc_html__('Posted in: ','Webbyrom'); ?></span><?php printf(('<span> %1$s</span>'),get_the_category_list( ', ' ));  ?>
                                        </div>
                                    <?php endif; ?>       
                                </header><!-- entry-header  --> 

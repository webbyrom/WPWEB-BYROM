...
class Cms_Image_Widget extends WP_Widget {
    ...
    function __construct() {
        parent::__construct(
            'cms_image', // Base ID
            esc_html__( 'Cms Contact', 'Webbyrom' ), // Name
            array(
                'description' => esc_html__( 'Add image with some options and optional link.', 'Webbyrom' ),
    ...
    function form( $instance ) {
        ...
        $image_holder = $this->get_field_id( 'images' ) . '-' . Webbyrom_generate_uiqueid();
        $title      = isset( $instance['title'] ) ? esc_attr( $instance['title'] ) : '';
        $image      = intval( $instance['image'] );
        ?>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>"><?php esc_html_e( 'Title:', 'Webbyrom' ); ?></label>
            <input class="widefat" id="<?php echo esc_attr( $this->get_field_id( 'title' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'title' ) ); ?>" type="text" value="<?php echo esc_attr( $title ); ?>" />
        </p>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'align' ) ); ?>"><?php esc_html_e( 'Align:', 'Webbyrom' ); ?></label>
            <select class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'align' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'align' ) ); ?>">
                <option value=""><?php esc_html_e( 'Default', 'Webbyrom' ); ?></option>
                <option value="left" <?php selected( "left", $instance['align'] ); ?>><?php esc_html_e( 'Left', 'Webbyrom' ); ?></option>
                <option value="center" <?php selected( "center", $instance['align'] ); ?>><?php esc_html_e( 'Center', 'Webbyrom' ); ?></option>
                <option value="right" <?php selected( "right", $instance['align'] ); ?>><?php esc_html_e( 'Right', 'Webbyrom' ); ?></option>
            </select>
        </p>
        <div class="redexp-widget-image">
            <label><?php esc_html_e( 'Image', 'Webbyrom' ); ?></label>
            ...
            ?></ul>
            <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'size' ) ); ?>"><?php esc_html_e( 'Image Size:', 'Webbyrom' ); ?></label>
            <select class="widefat" name="<?php echo esc_attr( $this->get_field_name( 'size' ) ); ?>" id="<?php echo esc_attr( $this->get_field_id( 'size' ) ); ?>">
                <?php
                    $image_sizes = apply_filters( 'image_size_names_choose', array(
                        'thumbnail' => esc_html__( 'Thumbnail', 'Webbyrom' ),
                        'medium'    => esc_html__( 'Medium', 'Webbyrom' ),
                        'large'     => esc_html__( 'Large', 'Webbyrom' ),
                        'full'      => esc_html__( 'Full Size', 'Webbyrom' )
                    ) );
            ...
            <p class="howto"><?php echo esc_html__( 'Add image size string, defaults include "thumbnail", "medium", "large", "full", or your defined size.', 'Webbyrom' ); ?></p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'link' ) ); ?>"><?php echo esc_html__( 'Link', 'Webbyrom' ); ?></label>
            ...
            <p class="howto"><?php echo esc_html__( 'Add link for image', 'Webbyrom' ); ?></p>
            <p>
                <label for="<?php echo esc_attr( $this->get_field_id( 'link_title' ) ); ?>"><?php echo esc_html__( 'Link Title', 'Webbyrom' ); ?></label>
        ...
        <p>
            <input id="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>" name="<?php echo esc_attr( $this->get_field_name( 'target' ) ); ?>" type="checkbox" value="1" <?php checked( "1", $instance['target'] );  ?>/><label for="<?php echo esc_attr( $this->get_field_id( 'target' ) ); ?>"><?php esc_html_e( 'Open link in new tab?', 'Webbyrom' ); ?></label>
        </p>
        <p>
            <label for="<?php echo esc_attr( $this->get_field_id( 'description' ) ); ?>"><?php esc_html_e( 'Description:', 'Webbyrom' ); ?></label>
...
function Webbyrom_register_recent_image_widget(){
    register_ef4_widget('Cms_Image_Widget');
}
add_action( 'widgets_init', 'Webbyrom_register_recent_image_widget' );
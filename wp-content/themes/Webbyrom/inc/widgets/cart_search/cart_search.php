...
class Webbyrom_Widget_Cart_Search extends WP_Widget
{
    protected $settings;
    ...
    {
        extract(array(
            'id_base'        => 'Webbyrom_widget_cart_search',
            'name'           => esc_html__('Cart & Search', 'Webbyrom'),
            'widget_options' => array(
                'description' => esc_html__('Display the user\'s Cart and Search form in the sidebar.', 'Webbyrom')
            )
        ));
        ...
        $this->settings = array(
            'title'                    => array(
                'type'  => 'text',
                'std'   => esc_html__('Cart & Search', 'Webbyrom'),
                'label' => esc_html__('Title', 'Webbyrom')
            ),
            'show_cart'                => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => esc_html__('Show Cart', 'Webbyrom'),
            ),
            'show_cart_contents_count' => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => esc_html__('Show Count', 'Webbyrom'),
            ),
            'show_cart_subtotal'       => array(
                'type'  => 'checkbox',
                'std'   => 0,
                'label' => esc_html__('Show Subtotal', 'Webbyrom'),
            ),
            'show_search'              => array(
                'type'  => 'checkbox',
                'std'   => 1,
                'label' => esc_html__('Show Search', 'Webbyrom'),
            ),
            'search_bar_style'         => array(
                'type'    => 'select',
                'std'     => 'dropdown',
                'options' => array(
                    'dropdown' => esc_html__('Dropdown', 'Webbyrom'),
                    'popup'    => esc_html__('Popup', 'Webbyrom')
                ),
                'label'   => esc_html__('Search Bar Style', 'Webbyrom'),
            ),
            'add_class'                => array(
                'type'  => 'text',
                'std'   => '',
                'label' => esc_html__('Add Class', 'Webbyrom')
            )
        );
    ...
    {
        wp_enqueue_script('wpWebbyrom-widget_cart_search_scripts', get_template_directory_uri() . '/inc/widgets/cart_search/cart_search.js', array('jquery'), '1.0.1', true);
        wp_enqueue_style('wpWebbyrom-widget_cart_search_scripts', get_template_directory_uri() . '/inc/widgets/cart_search/cart_search.css');
    }
...
function Webbyrom_register_cart_search_widget()
{
    register_ef4_widget('Webbyrom_Widget_Cart_Search');
}

add_action('widgets_init', 'Webbyrom_register_cart_search_widget');

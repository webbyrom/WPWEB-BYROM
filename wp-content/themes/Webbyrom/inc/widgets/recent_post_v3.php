...
function Webbyrom_register_recent_post_widget_v3(){
    register_ef4_widget('CS_Recent_Post_Widget_V3');
}
add_action( 'widgets_init', 'Webbyrom_register_recent_post_widget_v3' );

class CS_Recent_Post_Widget_V3 extends WP_Widget {
 
    function __construct() {
        parent::__construct(
            'cs_recent_post_v3',esc_html__( 'CS Recent Posts V3', 'Webbyrom' ),array('description' => esc_html__( 'Recent Posts Widget 3.', 'Webbyrom' )) 
        );
    ...
    function widget($args, $instance) {
        ...
        $title = apply_filters('widget_title', empty($instance['title']) ? esc_html__('Recent Posts', 'Webbyrom' ) : $instance['title'], $instance, $this->id_base);
                ...
                <div class="cms-recent-post">
                    <div class="cms-recent-post-wrapper">
                        ...
                        <div class="widget-recent-item clearfix">
                            ...
                            $thumbnail = get_the_post_thumbnail(get_the_ID(),'Webbyrom-770-645');
                            ?>
                                <div class="entry-thumbnail"> 
                                    <a href="<?php the_permalink(); ?>" class="img"> 
                                      <?php echo Webbyrom_html($thumbnail); ?> 
                                    </a> 
                                </div>
                               <?php
                                }else {
                            ...
                            <div class="entry-bottom">
                                <?php if ($show_decs) { ?>
                                    <p class="description"><?php echo Webbyrom_grid_limit_words( strip_tags( get_the_excerpt() ),15); ?></p>
                                <?php  } ?>
                            </div>  
                        </div> 
                        <?php endwhile; ?>
                    ...
                    <?php if ($link): ?>
                        <a class= "btn-recent btn btn-theme-default btn-round" href=" <?php echo esc_html($link); ?> " ><?php echo esc_html__('All News','Webbyrom'); ?></a>
                    <?php endif ?>
                </div>
            <?php 
             //wp_reset_postdata(); 
    ...
    function form($instance) {
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('show_date')); ?>"><?php esc_html_e( 'Show date:', 'Webbyrom' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('show_decs')); ?>"><?php esc_html_e( 'Show Description:', 'Webbyrom' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('show_image')); ?>"><?php esc_html_e( 'Show Image:', 'Webbyrom' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('show_author')); ?>"><?php esc_html_e( 'Show Author:', 'Webbyrom' ); ?></label>
        ...
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('number')); ?>"><?php esc_html_e( 'Number of posts to show:', 'Webbyrom' ); ?></label>
            <input id="<?php echo esc_attr( $this->get_field_id('number') ); ?>" name="<?php echo esc_attr( $this->get_field_name('number') ); ?>" type="text" value="<?php echo esc_attr( $number ); ?>" size="3" />
        </p>
        <p>
            <label for="<?php echo esc_attr($this->get_field_id('link')); ?>"><?php esc_html_e( 'Link:', 'Webbyrom' ); ?></label>

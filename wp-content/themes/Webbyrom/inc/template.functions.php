 ...
 * get theme option
 */
function Webbyrom_get_theme_option(){
 ...
 * get meta option
 */
function Webbyrom_get_meta_option(){
...
function Webbyrom_header_logo(){
    ...
    echo '</div>';
    Webbyrom_header_sticky_logo();
}

/**
 * get theme logo.
 */
function Webbyrom_header_sticky_logo(){
    global $opt_theme_options;
...
function Webbyrom_header_logo1(){
    global $opt_theme_options, $opt_meta_options;
        $has_sticky_logo =  !empty($opt_theme_options['sticky_logo']['url']) ? 'has-sticky-logo' : ''; 
    echo '<div class="main_logo '.esc_attr($has_sticky_logo).'">';
...
function Webbyrom_header_logo2(){
    global $opt_theme_options, $opt_meta_options;
    ...
    echo '</div>';
    Webbyrom_header_sticky_logo1();
}

function Webbyrom_header_sticky_logo1(){
    global $opt_theme_options;
 ...
 */
function Webbyrom_header_layout_class($class = ''){
    global $opt_theme_options,$opt_meta_options;
...
function Webbyrom_archive_sidebar(){
    global $opt_theme_options;
...
function Webbyrom_archive_class(){
    global $opt_theme_options;
 ...
 * get header class.
 */
function Webbyrom_header_class($class = ''){
    global $opt_theme_options;
 ...
 * main navigation.
 */
function Webbyrom_header_navigation(){
                                ...
                                <div class=" col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                    <div class="page-title-wrap <?php echo esc_attr($align);?>">
                                        <div class="page-title-text">
                                            <h1><?php Webbyrom_get_page_title(); ?></h1>
                                        </div> 
                                        <div class="breadcrumb-text"><?php Webbyrom_get_bread_crumb(); ?></div>
                                    </div>
                                </div>
                    <?php break;
                        case '3':?>
                    ...
                    <?php break;
                         case '4':?>
                                    <div class="col-xs-12 col-sm-12 col-md-12 col-lg-12">
                                        <div class="page-title-wrap <?php echo esc_attr($align);?> ">
                                            <div class="row">
                                                <div id="page-title-text" class="page-title-text col-xs-12 col-sm-12 col-md-6 col-lg-6"><h1><?php Webbyrom_get_page_title(); ?></h1></div>
                                                <div id="breadcrumb-text" class="breadcrumb-text col-xs-12 col-sm-12 col-md-6 col-lg-6">
                                                    <div class="breadcrumb-wrap">
                                                        <?php Webbyrom_get_bread_crumb(); ?>
                                                    </div>
                                                </div>
                                            </div>         
                                        </div>
 ...
 * page title
 */
function Webbyrom_get_page_title(){

    global $opt_meta_options;
    if(is_home()){
        if(is_front_page()){ 
            esc_html_e('Our Blog', 'Webbyrom');
        } else { 
    ...
    elseif (!is_archive()){
        ...
        /* search */
        elseif (is_search()):
            printf( esc_html__( 'Search Results for: %s', 'Webbyrom' ), '<span>' . get_search_query() . '</span>' );
        /* 404 */
        elseif (is_404()):
            esc_html_e( '404 Not Found', 'Webbyrom');
        /* other */
        else :
            the_title();
        endif;
        ...
        /* author. */
        elseif ( is_author() ) :
            printf( esc_html__( 'Author: %s', 'Webbyrom' ), '<span class="vcard">' . get_the_author() . '</span>' );
        /* date */
        elseif ( is_day() ) :
            printf( esc_html__( 'Day: %s', 'Webbyrom' ), '<span>' . get_the_date() . '</span>' );
        elseif ( is_month() ) :
            printf( esc_html__( 'Month: %s', 'Webbyrom' ), '<span>' . get_the_date() . '</span>' );
        elseif ( is_year() ) :
            printf( esc_html__( 'Year: %s', 'Webbyrom' ), '<span>' . get_the_date() . '</span>' );
        /* post format */
        elseif ( is_tax( 'post_format', 'post-format-aside' ) ) :
            esc_html_e( 'Asides', 'Webbyrom' );
        elseif ( is_tax( 'post_format', 'post-format-gallery' ) ) :
            esc_html_e( 'Galleries', 'Webbyrom');
        elseif ( is_tax( 'post_format', 'post-format-image' ) ) :
            esc_html_e( 'Images', 'Webbyrom');
        elseif ( is_tax( 'post_format', 'post-format-video' ) ) :
            esc_html_e( 'Videos', 'Webbyrom' );
        elseif ( is_tax( 'post_format', 'post-format-quote' ) ) :
            esc_html_e( 'Quotes', 'Webbyrom' );
        elseif ( is_tax( 'post_format', 'post-format-link' ) ) :
            esc_html_e( 'Links', 'Webbyrom' );
        elseif ( is_tax( 'post_format', 'post-format-status' ) ) :
            esc_html_e( 'Statuses', 'Webbyrom' );
        elseif ( is_tax( 'post_format', 'post-format-audio' ) ) :
            esc_html_e( 'Audios', 'Webbyrom' );
        elseif ( is_tax( 'post_format', 'post-format-chat' ) ) :
            esc_html_e( 'Chats', 'Webbyrom' );
        /* woocommerce */
 ...
 * @since 1.0.0
 */
function Webbyrom_get_bread_crumb() {
...
function Webbyrom_my_search_form( $form ) {
    $form = '<form method="get" action="'. esc_url( home_url( '/'  ) ).'" class="searchform search-form">
            <div class="form-group">
                <input type="text" value="' . get_search_query() . '" name="s" class="form-control" placeholder="'.esc_html__("Search",'Webbyrom').'" id="modal-search-input">
            </div>
...
add_filter( 'get_search_form', 'Webbyrom_my_search_form' );
                ...
                <ul class="single_detail">   
                    ...
                    <?php if(!isset($opt_theme_options['single_comment']) || (isset($opt_theme_options['single_comment']) && $opt_theme_options['single_comment'])): ?>
                        ...
                        if ( '1' === $comments_number ) {?>
                            <li class="detail-comment"><a href="<?php the_permalink(); ?>"><?php printf( _x( '1 comment', 'comments title', 'Webbyrom' ), get_the_title() ); ?></a></li>  
                       <?php }else{?>
                     <li class="detail-comment"><a href="<?php the_permalink(); ?>"><?php printf( _nx( '%1$s comment ', '%1$s comments', $comments_number, 'comments title', 'Webbyrom' ), number_format_i18n( $comments_number ), get_the_title());?>
                        </a></li>
                        <?php } ?>
                    <?php endif; ?>
                </ul>
...
function Webbyrom_post_detail_before(){
    global $opt_theme_options;
    $single_year  = get_the_time('Y'); 
    $single_month = get_the_time('F'); 
    $single_day   = get_the_time('d');
                    ...
                    <ul class="single_detail">   
                        ...
                        <?php if(isset($opt_theme_options['single_author']) && ($opt_theme_options['single_author'] == 1)): ?>
                            <li class="author-detail">
                                <span><?php echo esc_html__('Posted in ','Webbyrom'); ?> <?php echo esc_attr(get_the_author()); ?></span>
                            </li>
                        <?php endif; ?>
                    </ul>
                    <?php if(isset($opt_theme_options['single_author']) && ($opt_theme_options['single_author'] == 1)): ?>
...
function Webbyrom_post_detail_after(){
    global $opt_theme_options;
    ?>
    <?php   if( (isset($opt_theme_options['single_categories']) && $opt_theme_options['single_categories']) || (isset($opt_theme_options['single_comment']) && $opt_theme_options['single_comment'])): ?>
            <div class="entry-meta">
                <ul class="single_detail">    
                    <li class="detail-title"><?php echo esc_html__('in','Webbyrom') ?></li>
                    <?php if(has_category() && (!isset($opt_theme_options['single_categories']) || (isset($opt_theme_options['single_categories']) && $opt_theme_options['single_categories']))): ?>
...
function Webbyrom_post_related($category) {   
    if($category){  
    ...
    ?>
    <h5 class="related-title"><?php echo esc_html__('RELATED ARTICLES','Webbyrom'); ?></h5>
                ...
                <div class="post-related-item">
                    <div class="related-item-wrap clearfix">
                        ...
                        <div class="item-title">
                            <?php Webbyrom_post_detail_related(); ?>
                            <h6>
...
function Webbyrom_post_detail_related(){
    global $opt_theme_options;
    $single_year  = get_the_time('Y'); 
    $single_month = get_the_time('F'); 
    $single_day   = get_the_time('d');
            ...
            <div class="entry-meta-related">
                <ul class="single_detail">   
                    ...
                    <?php if(!isset($opt_theme_options['single_comment']) || (isset($opt_theme_options['single_comment']) && $opt_theme_options['single_comment'])): ?>
                        ...
                        $comments_number = get_comments_number();
                        if ( '1' === $comments_number ) {?>
                            <li class="detail-comment"><a href="<?php the_permalink(); ?>"><i class="fa fa-comments-o"></i><?php printf( _x( '1', 'comments title', 'Webbyrom' ), get_the_title() ); ?></a></li>  
                       <?php }else{?>
                        <li class="detail-comment"><a href="<?php the_permalink(); ?>"><i class="fa fa-comments-o"></i><?php printf( _nx( '%1$s', '%1$s', $comments_number, 'comments title', 'Webbyrom' ), number_format_i18n( $comments_number ), get_the_title());?>
                        </a></li>
                        <?php } ?>
                    <?php endif; ?>
                </ul>
 ...
 * @since 1.0.0
 */
function Webbyrom_post_like_callback(){
    global $smof_data;
    ...
    $text = '';
    if($likes){
        if($likes > 1) $text = esc_html__(' Likes','Webbyrom'); else $text = esc_html__(' Like','Webbyrom');
    }
...
add_action('wp_ajax_cms_post_like', 'Webbyrom_post_like_callback');
add_action('wp_ajax_nopriv_cms_post_like', 'Webbyrom_post_like_callback');


/*/ convert dates to readable format /*/
if (!function_exists('Webbyrom_relative_time')) {
    function Webbyrom_relative_time($a) {
        ...
        if (is_numeric($d) && $d > 0) {
            ...
            if ($d < 3)
                return esc_html__('right now','Webbyrom');
            //if less then minute
            if ($d < $minute)
                return floor($d) . esc_html__(' seconds ago','Webbyrom');
            //if less then 2 minutes
            if ($d < $minute * 2)
                return esc_html__('1 minute ago','Webbyrom');
            //if less then hour
            if ($d < $hour)
                return floor($d / $minute) . esc_html__(' minutes ago','Webbyrom');
            //if less then 2 hours
            if ($d < $hour * 2)
                return esc_html__('1 hour ago','Webbyrom');
            //if less then day
            if ($d < $day)
                return floor($d / $hour) . esc_html__(' hours ago','Webbyrom');
            //if more then day, but less then 2 days
            if ($d > $day && $d < $day * 2)
                return esc_html__('yesterday','Webbyrom');
            //if less then year
            if ($d < $day * 365)
                return floor($d / $day) . esc_html__(' days ago','Webbyrom');
            //else return more than a year
            return esc_html__('over a year ago','Webbyrom');
        }
    }
}
/**
 ...
 * @since 1.0.0
 */
function Webbyrom_get_image_crop($size) { 
...
function Webbyrom_get_image_croped($img_id,$size){ 
    if (function_exists('wpb_getImageBySize')){
        if(!empty($img_id)){
            $img = wpb_getImageBySize( array(
                'attach_id'  => $img_id,
...
function Webbyrom_archive_link(){
...
function Webbyrom_post_link_archive() {
    global $opt_meta_options;
 ...
 */
function Webbyrom_post_gallery($size){
    global $opt_theme_options, $opt_meta_options;
 ...
 * Return the thumbnail be cropped
 */
function Webbyrom_getImageCroped($img_id, $size){ 
    if (function_exists('wpb_getImageBySize')){
        $img = wpb_getImageBySize( array(
            'attach_id'  => $img_id,
            'thumb_size' => $size,
...
function Webbyrom_post_thumbnail($img_size='') {
...
function Webbyrom_post_archive_before(){
    global $opt_theme_options;
...
function Webbyrom_post_archive_after(){
    global $opt_theme_options;
    ?>
            ...
            <div class="entry-meta">
                <ul class="archive_archive">    
                    ...
                    <?php if(!isset($opt_theme_options['archive_comment']) || (isset($opt_theme_options['archive_comment']) && $opt_theme_options['archive_comment'])): ?>
                        ...
                        $comments_number = get_comments_number();
                        if ( '1' === $comments_number ) {?>
                            <li class="archive-comment"><a href="<?php the_permalink(); ?>"><?php printf( _x( '1 comment', 'comments title', 'Webbyrom' ), get_the_title() ); ?></a></li>  
                       <?php }else{?>
                     <li class="archive-comment"><a href="<?php the_permalink(); ?>"><?php printf( _nx( '%1$s comment ', '%1$s comments', $comments_number, 'comments title', 'Webbyrom' ), number_format_i18n( $comments_number ), get_the_title());?>
                        </a></li>
                        <?php } ?>
                    <?php endif; ?>
                </ul>
 ...
 */
function Webbyrom_post_sharing(){
    global $opt_theme_options;
                ...
                <div class="entry-share">
                    <ul class="social-share list-unstyled">
                        <?php if(isset($opt_theme_options['share-facebook']) && $opt_theme_options['share-facebook']==1): ?>
                            <li><a class="share-facebook" target="_blank" href="https://www.facebook.com/sharer/sharer.php?u=<?php the_permalink();?>"><i class="fa fa-facebook"></i> <?php echo esc_html__('Facebook','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-twitter']) && $opt_theme_options['share-twitter']==1): ?>
                            <li><a class="share-twitter" target="_blank" href="https://twitter.com/home?status=<?php esc_html_e('Check out this article', 'Webbyrom');?>:%20<?php echo strip_tags(get_the_title());?>%20-%20<?php the_permalink();?>"><i class="fa fa-twitter"></i><?php echo esc_html__('Twitter','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-googleplus']) && $opt_theme_options['share-googleplus']==1): ?>
                            <li><a class="share-google" target="_blank" href="https://plus.google.com/share?url=<?php the_permalink();?>"><i class="fa fa-google-plus"></i><?php echo esc_html__('Google Plus','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-linkedin']) && $opt_theme_options['share-linkedin']==1): ?>
                            <li><a class="share-linkedin" target="_blank" href="https://www.linkedin.com/shareArticle?mini=true&amp;url=<?php the_permalink();?>"><i class="fa fa-linkedin"></i><?php echo esc_html__('Linkedin','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-pinterest']) && $opt_theme_options['share-pinterest']==1): ?>
                            <li><a class="share-pinterest" target="_blank" href="https://pinterest.com/pin/create/button/?url=<?php the_permalink();?>"><i class="fa fa-pinterest"></i><?php echo esc_html__('Pinterest','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-tumblr']) && $opt_theme_options['share-tumblr']==1): ?>
                            <li><a class="share-tumblr" target="_blank" href="http://www.tumblr.com/share/link?url=<?php the_permalink();?>&amp;name=<?php echo strip_tags(get_the_title());?>&amp;description=<?php echo strip_tags(get_the_excerpt());?>"><i class="fa fa-tumblr"></i><?php echo esc_html__('Tumblr','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-vk']) && $opt_theme_options['share-vk']==1): ?>
                            <li><a class="share-vk" target="_blank" href="https://vk.com/share.php?url=<?php the_permalink();?>"><i class="fa fa-vk"></i><?php echo esc_html__('VKontakte','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-xing']) && $opt_theme_options['share-xing']==1): ?>
                            <li><a class="share-xing" target="_blank" href="https://www.xing-share.com/app/user?op=share;sc_p=xing-share;url=<?php the_permalink();?>"><i class="fa fa-xing"></i><?php echo esc_html__('Xing','Webbyrom'); ?></a></li>
                        <?php endif;?>
                        <?php if(isset($opt_theme_options['share-reddit']) && $opt_theme_options['share-reddit']==1): ?>
                            <li><a class="share-reddit" target="_blank" href="http://www.reddit.com/submit?url=<?php the_permalink();?>&amp;title=<?php echo strip_tags(get_the_title());?>"><i class="fa fa-reddit"></i><?php echo esc_html__('Reddit','Webbyrom'); ?></a></li>
                        <?php endif;?>
                    </ul>
                </div>
            </div>
...
function Webbyrom_portfolio_sharing(){
    global $opt_meta_options;
    if(isset($opt_meta_options['share_enable']) && $opt_meta_options['share_enable'] == 1): ?>
        <div class="portfolio-share">
            <ul class="social-share list-unstyled">
                <li class="sharing-title"><?php echo esc_html__('SHARE: ','Webbyrom'); ?></li>
                ...
                <?php if(isset($opt_meta_options['share-twitter']) && $opt_meta_options['share-twitter']==1): ?>
                    <li class="share-social"><a target="_blank" href="https://twitter.com/home?status=<?php esc_html_e('Check out this article', 'Webbyrom');?>:%20<?php echo strip_tags(get_the_title());?>%20-%20<?php the_permalink();?>"><i class="fa fa-twitter"></i></a></li>
                <?php endif; ?>
...
function Webbyrom_product_sharing(){
    ?>
    <div class="product-sharing">
        <a class="twitter" target="_blank" href="https://twitter.com/home?status=<?php esc_html_e('Check out this article', 'Webbyrom');?>:%20<?php echo strip_tags(get_the_title());?>%20-%20<?php the_permalink();?>"><i aria-hidden="true" class="fa fa-twitter"></i></a>
        ...
        <a class="linkedin" title="<?php esc_html_e('Share this article to Linkedin','Webbyrom'); ?>" target="_blank" href="https://linkedin.com/shareArticle?url=<?php the_permalink();?>"><i class="fa fa-linkedin"></i></a>
    </div>
    <?php
}

function Webbyrom_post_author(){
    global $opt_theme_options;
...
function Webbyrom_post_sidebar(){
    global $opt_theme_options;
 ...
 */
function Webbyrom_archive_author(){
    global $opt_theme_options;
    if(!isset($opt_theme_options['archive_author']) || (isset($opt_theme_options['archive_author']) && $opt_theme_options['archive_author'] == 1)): ?>
            <div class="detail-author">
                ...
                <div class="author-wrap">
                    <?php echo esc_html__('Posted in','Webbyrom'); ?> <?php the_author_posts_link(); ?>
                </div>
            </div>
    <?php endif; 
}
function Webbyrom_archive_readmore() {
    global $opt_theme_options;
    if(!isset($opt_theme_options['archive_readmore']) || (isset($opt_theme_options['archive_readmore']) && $opt_theme_options['archive_readmore'])): ?>
        <footer class="entry-footer">
            <a class="btn btn-theme-primary" href="<?php the_permalink(); ?>"><?php esc_html_e('Read More', 'Webbyrom') ?><i class="fa fa-angle-double-right"></i></a>
        </footer><!-- .entry-footer -->
    <?php  endif;
}

/* archive detail */
function Webbyrom_archive_detail(){
    global $opt_theme_options;
    $archive_year  = get_the_time('Y'); 
    $archive_month = get_the_time('F'); 
    $archive_day   = get_the_time('d'); 
                ...
                <div class="entry-meta">
                    <ul class="archive_detail">
                        ...
                        <?php if(!isset($opt_theme_options['archive_comment']) || (isset($opt_theme_options['archive_comment']) && $opt_theme_options['archive_comment'])): ?>
                            ...
                            $comments_number = get_comments_number();
                            if ( '1' === $comments_number ) {?>
                                <li class="detail-comment"><a href="<?php the_permalink(); ?>"><span class="fa fa-comments"></span><?php printf( _x( '1 comment', 'comments title', 'Webbyrom' ), get_the_title() ); ?></a></li>  
                           <?php }else{?>
                                 <li class="detail-comment"><a href="<?php the_permalink(); ?>"><span class="fa fa-comments"></span><?php printf( _nx( '%1$s comment ', '%1$s comments', $comments_number, 'comments title', 'Webbyrom' ), number_format_i18n( $comments_number ), get_the_title());?>
                            </a></li>
                            <?php } ?>
                        <?php endif; ?>
...
function Webbyrom_archive_detail1(){
    global $opt_theme_options;
    $archive_year  = get_the_time('Y'); 
    $archive_month = get_the_time('F'); 
    $archive_day   = get_the_time('d'); 
...
add_filter( 'body_class', 'Webbyrom_body_extra_class' );
function Webbyrom_body_extra_class( $classes ) {
    global $opt_theme_options,$opt_meta_options;
...
function Webbyrom_general_class(){
    global $opt_theme_options,$opt_meta_options;
    $classes = '';
    if( !empty($opt_theme_options['general_layout']) && $opt_theme_options['general_layout'] == '1' ){
        $classes = 'cs-boxed';
 ...
 * Show main sidebar
 **/
function Webbyrom_main_sidebar(){
...
function Webbyrom_footer_align(){
    global $opt_theme_options;
...
/* client logo footer */
function Webbyrom_client_logo_footer() {
    global $opt_theme_options;
    global $opt_meta_options;
...
function Webbyrom_footer_back_to_top(){
    global $opt_theme_options;
 ...
 */
add_action('after_setup_theme','Webbyrom_update_woo_number_item_in_page');
function Webbyrom_update_woo_number_item_in_page(){
    if(class_exists('EF4Framework')){
        if(class_exists( 'Woocommerce' )){
            add_filter( 'loop_shop_per_page', 'Webbyrom_woocommerce_number_columms', 20 );
        }
    }
}
/*
...
*/
function Webbyrom_woocommerce_number_columms(){
    global $opt_theme_options;
...
*/
add_action('wp_head','Webbyrom_maybe_fix_opt_meta_options',1);
function Webbyrom_maybe_fix_opt_meta_options()
{
    global $opt_meta_options;

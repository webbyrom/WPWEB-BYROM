 ...
 * config.
 */
add_filter('ef3-theme-options-opt-name', 'Webbyrom_set_demo_opt_name');

function Webbyrom_set_demo_opt_name(){
    return 'opt_theme_options';
}

add_filter('ef3-replace-content', 'Webbyrom_replace_content', 10 , 2);

function Webbyrom_replace_content($replaces, $attachment){
    return array(
...
add_filter('ef3-replace-theme-options', 'Webbyrom_replace_theme_options');

function Webbyrom_replace_theme_options(){
    return array(
        'dev_mode' => 0,
    );
}
add_filter('ef3-enable-create-demo', 'Webbyrom_enable_create_demo');

function Webbyrom_enable_create_demo(){
    return false;
}

add_action('ef3-import-finish', 'Webbyrom_set_woo_page');
/**
 * move post to trash
 */
add_action('ef3-import-start', 'Webbyrom_move_trash', 1);
if(!function_exists('Webbyrom_move_trash')){
    function Webbyrom_move_trash(){
        wp_trash_post(1);
        wp_trash_post(2);
    }
}
add_action('ef3-export-finish', 'Webbyrom_export_extra_option');
function Webbyrom_export_extra_option($folder_dir){
 ...
 * 
*/
add_action('ef3-import-start', 'Webbyrom_import_start', 10, 2);
function Webbyrom_import_start($id, $part){
    global $wp_filesystem;
    if ( class_exists('EF4Framework') ) {
        ...
        $data_content = preg_replace(
            array(
                '/http:\/\/dev\.joomexp\.com\/wordpress\/Webbyrom/',
            ), 
            site_url(), 
            $data_content
        );
 ...
 *
*/
add_action('ef3-import-finish', 'Webbyrom_import_extra_option',1,2);
function Webbyrom_import_extra_option($id,$folder_dir){
    $file = $folder_dir . 'extra-options.json';
 ...
 * @since 1.0.0
 */
function Webbyrom_set_options_page($id,$folder_dir){
    $opt_pages = array(
...
add_action('ef3-import-finish', 'Webbyrom_set_options_page');
/**
 * Crop image
 */
add_action('ef3-import-finish', 'Webbyrom_crop_images',99);
function Webbyrom_crop_images() {
    $query = array(
